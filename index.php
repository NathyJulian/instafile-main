<?php
$carpetaNombre = isset($_GET['MariaPJ']) ? $_GET['MariaPJ'] : '';
$carpetaRuta = "./descarga/" . $carpetaNombre;

// Verificar si el nombre de la carpeta está vacío y manejar el caso
if (empty($carpetaNombre)) {
    die("Error: El nombre de la carpeta no está especificado.");
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_FILES['archivo'])) {
            ...
            for ($i = 0; $i < count($archivos['name']); $i++) {
                ...
                if (move_uploaded_file($tmpArchivo, $carpetaRuta . '/' . $nombreArchivo)) {
...
<body>
    ...
    <div class="content">
        <h3>Sube tus archivos y comparte este enlace temporal: <span>proyecto0308.lat <?php echo htmlspecialchars($carpetaNombre); ?></span></h3>
        <div class="container">
            <div class="drop-area" id="drop-area">
                <form action="" id="form" method="POST" enctype="multipart/form-data">
                    ...
                    <label for="archivo" class="btn-seleccionar">Seleccionar archivo</label>
                    <input type="file" class="file-input" name="archivo[]" multiple id="archivo" onchange="document.getElementById('form').submit()">
            ...
            <div class="container2">
                <div id="file-list" class="pila">
                    <?php
                    // Verificar si la ruta de la carpeta es válida antes de usar scandir
                    if (!empty($carpetaRuta) && file_exists($carpetaRuta)) {
                        ...
                        echo "Error: La carpeta no es válida o no existe.";
                    }
                    ?>
                </div>
            </div>
